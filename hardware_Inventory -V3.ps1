...
Foreach ($s in $servers)
{
       ...
       $RegKey= $Reg.OpenSubKey("SOFTWARE\\ANZ\\BUILD\\SingleSource" )
       $ServerRoleID= $RegKey.GetValue("ServerRoleID")
       }
       $ServerRoleID
    ...
    $computerfreesize1 = $computerfreesize -join ','    
       Foreach ($CPU in $CPUInfo)
       {
              ...
              Add-Member -inputObject $infoObject -memberType NoteProperty -name "Registryvalue" -value $ServerRoleID  
